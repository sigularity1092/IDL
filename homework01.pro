...
for i=1,month do begin
  PRINT,x[i],'月的累计降水量为:',y1[i],'    有效值比率为:',y2[i]
endfor
;plot,x,y1,xtitle='Month',ytitle='Rain'
;oplot,x,y2
END
...
PRO soilmois,data,soilmoisture,feature
  ...
  ;cd控制上中下旬  
    ...
    IF cm EQ month THEN BEGIN
      IF ftenday EQ tenday THEN BEGIN
        ...
        ENDIF ELSE BEGIN
          ...
          tendaysoilmoisture+=soilmoisture[i]
        ENDELSE
          IF i EQ tnums-1 THEN BEGIN
    ...
    ENDELSE
  ENDFOR
  ;plot,x,y1,xtitle='tendays',ytitle='soilmoisture',xrange=[0,37]
  ;OPLOT,x,y2
END
